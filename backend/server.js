// ✅ server.js – שרת שיפוט טיפוס עם כל הפונקציונליות + שחזור חכם + סנכרון משופר
  ...
  try {
    ...
    for (const name in tempMemory) {
      ...
      for (const route in tempMemory[name]) {
        ...
        // אם יש הצלחה, שומרים את כל ההיסטוריה עד כולל ההצלחה
        attemptsMemory[name][route] = lastTIndex === -1
          ? history
          : history.slice(0, lastTIndex + 1);
      }
    }
...
async function logToAttemptsSheet(name, route, result) {
  ...
  try {
    ...
    console.log(`✅ כתיבה ל-Atempts (${name}, מסלול ${route}, ניסיון ${attemptCount}) הושלמה`);
  } catch (err) {
...
// 👇 כל שאר הפונקציות נשארות כפי שהיו – ללא שינוי:
...
// Updated /history endpoint: always clear and restore memory before responding
app.get('/history', async (req, res) => {
  // Clear in-memory attempts
  for (const key in attemptsMemory) delete attemptsMemory[key];
  // Restore from AllAttempts sheet
  await restoreAttemptsMemory();

  // Then return the requested history
...
app.get('/live', async (req, res) => {
  try {
    ...
    const grouped = {};
    competitors.forEach(c => { grouped[c.category] = grouped[c.category] || []; grouped[c.category].push(c) });
...
// אחרי כל ה‐API routes
const buildPath = path.join(process.cwd(), 'build');
app.use(express.static(buildPath));

// נתב כל שאר ה-GETs ל־index.html, כדי לתמוך ב-React Router
app.get('/*', (req, res) => {
